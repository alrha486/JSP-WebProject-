...
<body>
	<%
		...
		if (session.getAttribute("id") != null) {
			...
			userPW = (String) session.getAttribute("pw");   // 세션 ID, PW 저장
		}
		if (userID == null) { // ID 값에 따른 예외처리
	%>
	<script type="text/javascript">
		alert("로그인 후 이용해주세요");
		history.back();
	...
	<%
		} else if (userID != null) { // ID 값에 따른 예외처리
	%>
	...
	<div class="container">
		<div class="row">  <!-- 게시 글 입력 폼-->
			<form name="postFrm" method="post" action="p_postProc.jsp"  
				...
				<table class="table table-striped"
					...
					<tr>
						...
						<input type="hidden" name="pass" value="<%=userPW%>">  <!--hidden 타입으로 유저 ID,PW 값도 같이 보내기 -->
